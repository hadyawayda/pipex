...
int	setup_pipes_and_forks(int infile, int outfile, char **argv, char **envp)
{
	...
	if (pipe(pipe_fd) == -1)
		return (exit_with_error("pipe", 1));
	pid1 = fork();
	if (pid1 < 0)
		return (exit_with_error("fork", 1));
	if (pid1 == 0)
	{
...
int	main(int argc, char **argv, char **envp)
{
	...
	if (infile < 0)
		return (exit_with_error("open infile", 1));
	outfile = open(argv[4], O_WRONLY | O_CREAT | O_TRUNC, 0644);
	if (outfile < 0)
		return (exit_with_error("open outfile", 1));
	if (argv[2][0] == '\0' || argv[3][0] == '\0')
		return (exit_with_error("One or both commands are missing", 1));
